...
var parts = [
    ...
    sunshineParts = [
      'Profiles', 'Person', 'People', 'Objects', 'Relationships'
    ];
...
exports.createClient = function (options) {
  ...
  if (nconf.get('subdomain')) {
    ...
    } else if (options.stores.defaults.store.sunshine){
      endpoint = '.zendesk.com/api/sunshine';
    } else {
  ...
  if (options.stores.defaults.store.helpcenter) {
    ...
    partsToAdd = sunshineParts;
    clientPath = './client/sunshine/';
  } else {
    partsToAdd = parts;
    clientPath = './client/';
  }
