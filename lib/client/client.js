...
Client.prototype.request = function (method, uri) {
  var url, self = this, res, args = Array.prototype.slice.call(arguments),
      ...
      customHeaders = this.options.get('customHeaders'); // global client-level headers to inclue with all API calls
  ...
  // Sunshine API Custom Object Record Update requires following PATCH Content-Type
  if (method === 'PATCH') {
    self.options.headers['Content-Type'] = 'application/merge-patch+json';
  }
  ...
  // support for localized sync/async Sunshine API calls
  if (this.options.get('sunshine') && body && body.customHeaders && body.customHeaders.hasOwnProperty('Z-Handle-Synchronously')) {
    self.options.headers['Z-Handle-Synchronously'] = body.customHeaders['Z-Handle-Synchronously'];
  }

  // support for global customHeaders in client create
  if (customHeaders) {
...
function checkRequestResponse(self, response, result, callback) {
  ...
  // handle Sunshine API responds with empty 202
  if (!result && response && response.statusCode !== 202) { // should this really be an error?
...
function assembleUrl(self, uri) {
  ...
  if ('object' === typeof uri && Array.isArray(uri)) {
    ...
    // Sunshine API doesn't allow .json to be appended to path
    if (self.options.get('remoteUri').includes('sunshine')){
      return self.options.get('remoteUri') + '/' + uri.join('/') + params;
    };
